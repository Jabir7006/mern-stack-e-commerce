   ...
   {whishListItems && whishListItems.map((product) => (
        ...
        <div className="flex w-2/5"> 
          <div className="w-full md:w-20">
            <img className="h-24 w-full md:object-cover" src={`${
        ...
        <div className="flex justify-center w-1/5">
          ...
          <input className="mx-2 border text-center w-8" type="text" defaultValue={1} disabled/>
         
        </div>
        <span className="text-center w-1/5 font-semibold text-sm">${product.price}</span>
        <button className="text-center w-1/5 font-semibold text-sm bg-[#111821] text-white py-3 px-5 rounded-md hover:bg-yellow-500 duration-200" onClick={() => handleAddToCart(product)}>Add To Cart</button>
      </div>
